...
def read_dataset():
    ...
    power = pd.read_csv('datasets/volume.csv', index_col=0, header=0)
    ...
    return occ, prc, adj, col, dis, cap, time,power,inf

def read_dataset1(): #read_dataset for UrbanEV
    occ = pd.read_csv('datasets_UrbanEV/occupancy.csv', index_col=0, header=0)
    inf = pd.read_csv('datasets_UrbanEV/information.csv', index_col=None, header=0)
    prc = pd.read_csv('datasets_UrbanEV/price.csv', index_col=0, header=0)
    adj = pd.read_csv('datasets_UrbanEV/adj.csv', index_col=0, header=0)  # check
    dis = pd.read_csv('datasets_UrbanEV/distance.csv', index_col=0, header=0)
    time = pd.read_csv('datasets_UrbanEV/time.csv', index_col=None, header=0)
    power = pd.read_csv('datasets_UrbanEV/volume.csv', index_col=0, header=0)
...
def division(data, train_rate, test_rate):
...
#         #output_prc_l = self.prc_l[idx, :].to(self.device)
#         return output_occ, output_prc, output_label#,output_prc_l



# class CreateDataset(Dataset):   #with random fake data
...
#
#         # 获取训练集1%样本的索引
#         self.train_indices_1_percent = random.sample(range(len(occ)), int(0.01 * len(occ)))
#
#     def __len__(self):
#         return len(self.occ)
#
...
#         output_label = self.label[idx, :].to(self.device)
#
#         # 如果当前样本在1%训练样本中，将 occ 的一个时序数据设为 0
#         if idx in self.train_indices_1_percent:
#             # 随机选择12个时序数据中的一个时序数据位置，并将其设为0
#             random_timestep = random.randint(0, output_occ.shape[0] - 1)
#             output_occ[random_timestep, :] = 0  # 将这个时序数据设为0
#
#         return output_occ, output_prc, output_label

# class CreateDataset(Dataset):   #with fix fake data
#     def __init__(self, occ, prc, lb, pt, device, adj):
#         occ, label = create_rnn_data(occ, lb, pt)
#         #print(occ.shape)
#         #print(label.shape)
...
#         self.device = device
#
#         # 获取训练集1%样本的索引（确定性选择，按照固定间隔）
#         total_samples = len(occ)
#         num_selected = int(0.01 * total_samples)
#         # 固定选择每隔 100 个样本中的第一个（例如）
#         self.train_indices_1_percent = list(range(0, total_samples, 100))[:num_selected]
#
#     def __len__(self):
#         return len(self.occ)
#
...
#         output_label = self.label[idx, :].to(self.device)
#
#         # 如果当前样本在1%训练样本中，将 occ 的一个时序数据设为0
#         if idx in self.train_indices_1_percent:
#             # 确定性选择固定的时间步（例如，选择第一个时间步）
#             random_timestep = 0  # 固定选择第0个时间步
#             output_occ[random_timestep, :] = 0  # 将这个时序数据设为0
#
#         return output_occ, output_prc, output_label
...
class PseudoDataset(Dataset):
    def __init__(self, occ, prc, lb, pt, device, adj, law, num_layers=2, prop=0.3):  # adj
        occ, label = create_rnn_data(occ, lb, pt)
        prc, _ = create_rnn_data(prc, lb, pt)
        self.occ = torch.Tensor(occ)
        self.prc = torch.Tensor(prc)
