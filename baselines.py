...
class VAR(nn.Module):
    def __init__(self, node=247, seq=12, feature=2):
...
class LassoVAR(nn.Module):
    def __init__(self, node=247, seq=12, feature=2):
        super(LassoVAR, self).__init__()
        self.linear = nn.Linear(node * seq * feature, node)
        self.lasso_coeff = nn.Parameter(torch.tensor([0.1]))
    ...
    def add_lasso_loss(self, criterion, outputs, labels):
        loss = criterion(outputs, labels)
        l1_regularization = torch.mean(torch.abs(self.linear.weight))
        total_loss = loss + self.lasso_coeff * l1_regularization
        return total_loss
...
class LstmGat(nn.Module):
    ...
    def forward(self, occ, prc):
        ...
        atts_mat = self.gat_l1(x)
        occ_conv1 = torch.matmul(atts_mat, x)
        occ_conv1 = self.dropout(self.LeakyReLU(self.gcn(occ_conv1)))

        atts_mat2 = self.gat_l2(occ_conv1)
...
class FGN(nn.Module):
    def __init__(self, pre_length=1, embed_size=64, feature_size=0, seq_length=12, hidden_size=32,
                 hard_thresholding_fraction=1, hidden_size_factor=1, sparsity_threshold=0.01):
        super().__init__()
